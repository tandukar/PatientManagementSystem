...
const Profile = () => {
  ...
  const { data: userDetail = [] } = useUserDetailQuery(adminId);
  const [updatePasswordRecep, { isLoading, error }] = useUpdatePasswordAdminMutation();

  useEffect(() => {
    const token = localStorage.getItem("token");
    const adminId = getIdFromLocalStorage(token);
    setadminId(adminId);
  }, []);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response  = await updatePasswordRecep(
        {
          id: adminId,
