...
import { useDoctorDetailQuery, useIpdQuery } from "../DoctorApiSlice";
...
import { useUpdateIpdStatusMutation } from "../DoctorApiSlice";
...
const createData = (name, admissionDate, roomNumber, bedNumber, action) => {
  return { name, admissionDate, roomNumber, bedNumber, action };
};
...
const Ipd = () => {
  ...
  const [updateAppointmentStatus] = useUpdateIpdStatusMutation();
  ...
  const rows = appointmentsSorted
    ? appointmentsSorted
        // .filter((appointment) => {
        //   const admissionDate = new Date(appointment.admissionDate);   //YO CODE LE CURRENT DATE KO GET GAREKO HO
  ...
  return (
    <>
      <div className="m-6 text-custom-blue text-xl font-bold">In-Patients</div>
